#' Fitting MLP to the EUPPBench visibility ensemble
#'
#' @name mlpProbsVis
#' @description Fitting MLP to the EUPPBench datasets' visibility forecasts.
    ...
    for (lead in leadTimes[1:nLeadTimes]) {
      ...
      predTmp <-
        foreach(
          ...
          test.aux <-
            cbind(data.test, rep(sin(2 * pi * d / 365), nStats), rep(cos(2 * pi * d /
                                                                           365), nStats))
          ...
          if (anyNA(mlpTest[, -1])) {
            ...
            probTmp[, dimnames(mlpTrain$obs)[[2]]] <-
